...
export function TaskItem({task}) {
  ...
  const {deleteTask, setTaskStatus, member} = useTasks();
  ...
  const getTaskLabel = () => {
    let _member = member.find(
      (el) =>
        el._id.valueOf().toString() === task.assignee.valueOf().toString(),
    );

    if (task.assignee != null && _member) {
      return task.name + `(${task.department.name})` + ' - ' + _member.name;
    }
